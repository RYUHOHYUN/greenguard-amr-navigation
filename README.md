...
전체 시스템은 AMR 자율주행, 고정 웹캠 기반 토마토 검사, TurtleBot4 카메라 기반 침입자 감지, YOLO 기반 객체 탐지, Mission Manager, SQLite3 DB, Flask Web UI로 구성되었습니다.
...
AMR Control은 팀 내에서 공동으로 진행되었습니다. 제 담당 범위는 map generation, waypoint collection, waypoint patrol route, and patrol-only navigation control이었습니다. 팀원은 순찰 중 객체 탐지 이벤트가 발생했을 때 비프음과 함께 tracking behavior로 전환되는 부분을 주로 구현했습니다.
...
* TurtleBot4 AMR: Nav2 기반 자율주행, 목표 위치 이동, 순찰 수행
* TurtleBot4 Camera: 침입자 감지 및 tracking behavior에 활용
* Fixed Webcam: 토마토 검사에 활용
* YOLO Detection: 토마토 및 침입자 객체 탐지
...
### Scenario 1: Patrol and Tomato Inspection

* Waypoint patrol
* Fixed webcam-based tomato inspection
* YOLO result logging
* Web UI display
* Return behavior
...
* TurtleBot4 camera-based intruder detection
* AMR movement toward the target area
