...
| Parameter | Before | After | Direction | Purpose |
|---|---:|---:|---|---|
| `minimum_travel_distance` | 0.0 | 0.15 | Increase | 너무 자주 scan node가 생성되는 것을 방지 |
| `minimum_travel_heading` | 0.0 | 0.12 | Increase | 아주 작은 회전에도 node가 생성되는 것을 방지 |
| `loop_search_maximum_distance` | 3.0 | 1.5 | Decrease | 너무 먼 loop closure 후보 제거 |
| `loop_match_minimum_chain_size` | 10 | 15 | Increase | 연속적으로 더 많이 맞아야 loop closure 인정 |
| `loop_match_maximum_variance_coarse` | 3.0 | 1.5 | Decrease | 불확실한 coarse match 제거 |
| `loop_match_minimum_response_coarse` | 0.35 | 0.50 | Increase | 1차 매칭 점수 기준 강화 |
| `loop_match_minimum_response_fine` | 0.45 | 0.60 | Increase | 최종 매칭 점수 기준 강화 |
