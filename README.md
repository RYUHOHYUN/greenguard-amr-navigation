...
AMR Control은 팀 내에서 공동으로 진행되었습니다. 제 담당 범위는 map generation, waypoint collection, waypoint patrol route, and patrol-only navigation control이었습니다. 팀원은 객체 탐지 이벤트가 발생했을 때 AMR을 특정 구역으로 출동시키고, 이후 추적 또는 복귀 동작으로 전환하는 이벤트 기반 동작을 주로 구현했습니다.
...
### Scenario 1: Patrol, Tomato Inspection, and Mode Transition

* TurtleBot4 waypoint patrol
* Robot-mounted laptop camera-based tomato inspection
* YOLO result logging
* Web UI display
* Event-based mode transition during patrol
* Return to patrol or predefined route after event handling

이 시나리오는 기존 patrol scenario와 patrol 중 mode transition scenario를 통합한 것입니다. TurtleBot4가 waypoint를 따라 순찰하면서 노트북 카메라 기반 토마토 검사를 수행하고, 필요 시 이벤트 처리 모드로 전환되는 흐름입니다.
