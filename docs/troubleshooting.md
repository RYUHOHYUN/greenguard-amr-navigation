...
* `minimum_travel_distance`: 0.0 → 0.15
* `minimum_travel_heading`: 0.0 → 0.12
* `loop_search_maximum_distance`: 3.0 → 1.5
* `loop_match_minimum_chain_size`: 10 → 15
* `loop_match_maximum_variance_coarse`: 3.0 → 1.5
* `loop_match_minimum_response_coarse`: 0.35 → 0.50
* `loop_match_minimum_response_fine`: 0.45 → 0.60
...
Navigation speed can affect perception stability in an integrated robot system. For patrol scenarios involving camera-based detection, the navigation speed should be tuned together with the perception requirements.
